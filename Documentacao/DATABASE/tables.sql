...
ALTER TABLE TB_FUNCIONARIOS ADD CONSTRAINT TB_FUNCIONARIOS_FK_EMPRESA FOREIGN KEY (EMPRESA) REFERENCES TB_EMPRESA (ID_EMPRESA) ENABLE;

-- Adição de Restrição de Chave Estrangeira entre TB_EMPRESA e TB_RAMO
ALTER TABLE TB_EMPRESA ADD CONSTRAINT TB_EMPRESA_FK_RAMO FOREIGN KEY (RAMO) REFERENCES TB_RAMO (ID_RAMO) ENABLE;

-- Remoção de Restrição de Chave Estrangeira em TB_FUNCIONARIOS
ALTER TABLE TB_FUNCIONARIOS DROP CONSTRAINT TB_FUNCIONARIOS_FK_EMPRESA;

-- Tabela de Doações
CREATE TABLE TB_DOACOES (
                            ID_DOACAO NUMBER(19,0),
                            VALOR NUMBER(10,2),
                            TIPO VARCHAR2(255),
                            DATA_DOACAO DATE,
                            EMPRESA_DOADORA NUMBER(19,0)
);

CREATE UNIQUE INDEX IDX_TB_DOACOES_ID ON TB_DOACOES (ID_DOACAO);

ALTER TABLE TB_DOACOES MODIFY (ID_DOACAO NOT NULL ENABLE);
ALTER TABLE TB_DOACOES ADD PRIMARY KEY (ID_DOACAO);

CREATE SEQUENCE SQ_DOACAO MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 1 CACHE 20 NOORDER NOCYCLE NOKEEP NOSCALE GLOBAL;

ALTER TABLE TB_DOACOES ADD CONSTRAINT TB_DOACOES_FK_EMPRESA FOREIGN KEY (EMPRESA_DOADORA) REFERENCES TB_EMPRESA (ID_EMPRESA) ENABLE;
